    ...
    async function loadFirstTimers() {
        ...
        try {
            ...
            if (data.success && data.data.length > 0) {
                ...
                <div onclick='openMemberDetail(${JSON.stringify(m).replace(/'/g, "\\'")})' 
                    ...
                    <div>
                        <div class="flex items-center gap-2 mb-1">
                            <h4 class="font-bold text-slate-800 group-hover:text-[#00BD06] transition-colors">${m.Full_Name.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</h4>
                    ...
                    <div class="flex gap-2" onclick="event.stopPropagation()">
                        ${m.Phone ? `
                        <a href="tel:${m.Phone.replace(/</g, '&lt;')}" class="w-10 h-10 bg-green-50 text-[#00BD06] rounded-full flex items-center justify-center hover:bg-[#00BD06] hover:text-white transition-colors">

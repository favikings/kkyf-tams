...
- **DB access:** only `db()` from `app/config/db.php` opens a connection. **Every** query is a PDO prepared statement — never string-interpolated SQL. Multi-step writes use transactions (`TECH_SPEC.md §4`). Soft-delete only, with two narrow, explicitly documented exceptions (see `doc/DECISIONS.md` R-37): a zero-attendance member may be hard-deleted from `member-view.php`, and a Super Admin may hard-delete every member in one tent from `tents.php`'s Danger Zone. Every other delete stays soft (`is_active`/`status = 'inactive'`) — don't add a third exception without recording it there first.

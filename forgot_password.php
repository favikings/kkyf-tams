...
<head>
    ...
    <title>Tent Manager | Forgot Password</title>
    ...
    <div class="flex min-h-screen w-full flex-col lg:flex-row">
        ...
        <div
            ...
            <div class="relative z-10 px-12 text-center">
                ...
                <h1 class="font-display text-4xl font-black tracking-tight text-white xl:text-5xl">
                    Password <span class="text-primary">Recovery</span>
                </h1>
        ...
        <div
            ...
            <div class="w-full max-w-md">
                ...
                <div class="mb-10">
                    <h3 class="font-display text-3xl font-extrabold text-deep-slate dark:text-white sm:text-4xl">Forgot
                        Password?</h3>
                    <p class="mt-2 text-slate-500 dark:text-slate-400">Enter your email to reset your password.</p>
                </div>

                <form class="space-y-6" onsubmit="handleForgot(event)">
                    <div>
    ...
    <script>
        async function handleForgot(e) {
            ...
            try {
                ...
                if (data.success) {
                    await StitchAlert.showSuccess('Link Sent!', 'Check your email for instructions.');

                    // SIMULATION: Show the link in an alert/console for testing
                    if (data.data.reset_link) {
                        console.log("SIMULATED EMAIL LINK:", data.data.reset_link);
                        // For user convenience in inspection
                        StitchAlert.showSuccess('Dev Mode', 'Check console for simulated link.');

                        // Option: Automatically redirect to the reset link after delay (for smooth flow demo)
                        // Or just let them check console.
                        // Let's create a temporary visible link on screen for ease
                        const linkContainer = document.createElement('div');
                        linkContainer.className = 'mt-4 p-4 bg-green-50 text-green-700 rounded-xl text-sm break-all';
                        linkContainer.innerHTML = `<strong>Dev Mode Link:</strong> <a href="${data.data.reset_link}" class="underline">Click to Reset</a>`;
                        document.querySelector('form').appendChild(linkContainer);
                    }
                } else {
                    StitchAlert.showError('Error', data.error || 'Failed to send link.');
                }
            } catch (err) {
                console.error(err);
